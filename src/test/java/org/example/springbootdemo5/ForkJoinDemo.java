...
public class ForkJoinDemo {
    ...
    void test5() throws ExecutionException, InterruptedException {
        ...
        ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(10, 20, 1000,
                TimeUnit.SECONDS, new ArrayBlockingQueue<>(100));
        for (int i = 0; i < 10; i++) {
            threadPoolExecutor.submit(() -> System.out.println(1/0));
        }
        ...
        for (Future future : futures) {
            System.out.println("每个线程所执行的结果："+future.get());
        }
    }
}
